...
| Item type | Route | Who |
|-----------|-------|-----|
| **Small, localized fixes** (a tighter `description`, a single before/after pair, a wording fix) | **direct** | The agent applies it directly, item by item. |
| **Substantial / cross-cutting** (a rule that spans several skills, a contradiction between skills) | **spec-first** | A skill spec (or revision plan) is written and reviewed, then applied. |

When in doubt, prefer the lighter route for a genuinely localized fix and the heavier route once a
fix touches multiple skills or the shape of a rule.
...
> 1. **Route by type** — small, localized fixes → direct (applied directly);
>    substantial/cross-cutting → spec-first (skill spec/revision plan written, reviewed, applied).
