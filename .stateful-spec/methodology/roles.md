...
> This repository (type **skills**) runs the single-agent model above. The upstream
> multi-agent flow (a software team of PM/Architect + Engineer taking turns through a
> shared state file) is out of scope here and is not copied into this project.

### Post-delivery QA

When defects surface after a delivery, the [Post-Delivery QA](qa-phase.md) loop reuses the
single-agent roles above: the agent registers each item, categorizes it (E/P/H), finds the
root cause, applies the fix, and folds the lesson back into a skill — register → categorize →
