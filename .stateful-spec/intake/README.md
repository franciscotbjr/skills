...
- **`QA/`** — defects and observations from using the skills (a skill firing when it should not,
  failing to fire, or giving bad guidance), before they are routed (small/localized → direct fix;
  substantial/cross-cutting → spec-first; see the canonical QA phase in `methodology/qa-phase.md`,
  which routes by type and applies the E/P/H discipline).
...
agent at the explicit triage moments** above — the folders are a shared convention, not an automated
queue.
