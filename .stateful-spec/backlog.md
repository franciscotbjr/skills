...
| O-005 | Revisar o catálogo existente sob os protocolos do `persona.md`: G1–G2/R1–R4 por skill (as 6 pré-persona) com veredito conforme/achado/INCONCLUSIVO, teste de colisão G3 por par do catálogo e lista triada de achados com opções tipadas. Sem alterar as skills nesta etapa — correções só após decisão do usuário (§8.6). | idea (`intake/Backlog/revisar-catalogo.md`) | promoted | iteração [008-revisar-catalogo](history/008-revisar-catalogo.md) |
| O-006 | Tratar sentenças argumentativas por negação em outputs de IAs generativas: analisar se é skill nova ou evolução de `prosa-sobria`/`sober-prose`, delimitar a sobreposição com o anti-padrão já coberto ("não é X, é Y") e especificar gatilho, idioma, contraparte e fundamentação. | idea (`intake/Backlog/sentencas-negativas.md`) | promoted | iteração [009-sentencas-negativas](history/009-sentencas-negativas.md) |
